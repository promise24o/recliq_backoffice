...
import SuspendedFlaggedAgentsTable from './components/SuspendedFlaggedAgentsTable';
import AgentRiskDetailDrawer from './components/AgentRiskDetailDrawer';
...
// Mock Data
const mockRiskAgents: RiskAgent[] = [
  {
    id: 'AGT001',
    name: 'Samuel Kamau',
    phone: '+2348012345678',
...
// Main Component
const SuspendedFlaggedAgentsPage: React.FC = () => {
  const [agents, setAgents] = useState<RiskAgent[]>(mockRiskAgents);
  ...
  const [selectedAgent, setSelectedAgent] = useState<RiskAgent | null>(null);
  ...
  const filteredAgents = agents.filter(agent => {
    ...
    const matchesRiskState = selectedRiskState === 'all' || agent.riskState === selectedRiskState;
    const matchesReason = selectedReason === 'all' || agent.reason.toLowerCase().includes(selectedReason.toLowerCase());
    
    return matchesSearch && matchesCity && matchesRiskState && matchesReason;
  });

  const handleAgentClick = (agent: RiskAgent) => {
    setSelectedAgent(agent);
    setIsDrawerOpen(true);
    // Log access for security
    console.log(`Risk profile accessed: ${agent.id} by ${'current_user'}`);
  };
  ...
  const handleRiskAction = (action: string, agent: RiskAgent, reason?: string, duration?: string) => {
    ...
    switch (action) {
      ...
      case 'extend_suspension':
        setNotification({
          open: true,
          message: `Suspension for ${agent.name} has been extended`,
          severity: 'warning'
        });
        break;
      case 'ban':
        setNotification({
          open: true,
          message: `Agent ${agent.name} has been permanently banned`,
          severity: 'error'
        });
        break;
      default:
  ...
  return (
    <PageContainer title="Suspended & Flagged Agents" description="Trust, risk, and enforcement">
      <Breadcrumb title="Suspended & Flagged Agents" subtitle="Trust, risk, and enforcement" />
      ...
      {/* Suspended & Flagged Agents Table */}
      <Box mt={3}>
        <SuspendedFlaggedAgentsTable
...
export default SuspendedFlaggedAgentsPage;
